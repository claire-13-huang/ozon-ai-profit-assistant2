...
## 2026-06-09 Feature / Topic Tag Assistant v0.1

- 修改目标：在 Function Center 中把 `主题标签助手` 从占位模块升级为可用的本地规则版主题标签诊断工具，不扩展其他未来模块。
- 涉及文件：index.html、css/style.css、js/main.js、docs/manual-test-cases.md、docs/DEVELOPMENT_LOG.md。
- 修改内容：新增主题标签助手输入区，支持商品标题、目标平台、类目、材质、使用场景、核心卖点、目标人群、商品证据包、竞品标签/标题和想避免的词；新增本地规则标签生成逻辑，输出核心标签、场景标签、属性标签、人群/用途标签、长尾标签、不建议使用的泛标签、三组标签组合方案，以及相关性、具体性、搜索覆盖、错配风险控制、转化意图评分；明确展示数据边界，说明建议不代表平台真实搜索量或官方标签库，俄语词需要卖家结合平台搜索结果再次确认。
- 验收方式：使用野餐保温包样例生成报告，应识别 `термосумка`、`сумка холодильник`、`пикник`、`кемпинг`、`пляж`、`большая вместимость`、`термоизоляция`、`карманы` 等方向，并将 `женская сумка`、`рюкзак`、`bag / сумка` 归入不建议使用；运行 `node --check`、`git diff --check`，并确认前端文件没有直接调用 `api-seller.ozon.ru`。
- 已知风险：v0.1 只使用本地保守规则和用户提供文本，不接入平台真实搜索量、官方标签库、翻译 API、LLM、爬虫或动态渲染；俄语长尾组合仍需要卖家在平台搜索结果中复核。

## 2026-06-09 Feature / Topic Tag Assistant v0.2

- 修改目标：把主题标签助手从分组解释升级为更贴近 Ozon 上架操作的标签规划器，输出可复制最终标签清单和逐条风险判断。
- 涉及文件：index.html、css/style.css、js/main.js、docs/manual-test-cases.md、docs/DEVELOPMENT_LOG.md。
- 修改内容：新增规格/型号/尺寸、竞品主题标签、我的候选标签、品牌词/禁用词输入；最终标签规划目标为 30 个，但不会用无关词硬凑；使用 `Array.from(tag).length` 检查每个标签不超过 30 个字符；品牌词、平台词、禁用词、重复词、泛词和错配词会从最终复制清单排除；新增一行一个和逗号分隔两种复制块、标签明细表、竞品标签分析、我的候选标签分析和不建议使用标签说明。
- 验收方式：野餐保温包样例应生成可复制清单，保留 `термосумка для пикника`、`сумка холодильник 35л` 等精准长尾；`сумка` 不作为强标签；`женская сумка`、`рюкзак`、`outdoor bag` 和包含品牌/平台禁用词的标签被排除或标记风险；运行 `node --check`、`git diff --check` 和前端 Ozon Seller API grep；浏览器验证无控制台错误。
- 已知风险：v0.2 仍是本地规则诊断，不接入真实 Ozon 标签库、搜索量、广告数据、订单数据或翻译服务；俄语表达和长尾组合仍需要卖家结合平台搜索结果确认。

## 2026-06-09 UX / Topic Tag Assistant v0.3

- 修改目标：修正 v0.2 的复制格式和报告密度，让最终 Ozon 标签清单成为报告首要结果。
- 涉及文件：index.html、css/style.css、js/main.js、docs/manual-test-cases.md、docs/DEVELOPMENT_LOG.md。
- 修改内容：最终标签统一格式化为 `#` 开头、多词用 `_` 连接、一行一个；移除逗号分隔复制块；最终可复制清单移到报告顶部；30 字符校验改为检查最终可见标签（包含 `#` 和 `_`）；标签明细表压缩为标签、类型、来源、字符数、状态、理由 6 列；拒绝标签按过泛、错配、禁用词、过长、重复意图等分组；竞品标签和我的候选标签分析改为短摘要；旧分组解释移入折叠详情。
- 验收方式：野餐保温包样例最终清单应显示 `#термосумка_для_пикника` 这类格式；`#женская_сумка`、`#рюкзак`、弱泛词 `#сумка` 和 unsupported `#outdoor_bag` 不进入最终清单；运行 `node --check`、`git diff --check`、前端 Ozon Seller API grep 和浏览器烟测。
- 已知风险：v0.3 仍是本地规则格式化和风险判断，不代表 Ozon 官方标签库或真实搜索量；俄语语义仍需卖家结合平台搜索结果确认。

## 2026-06-09 Cleanup / Topic Tag Assistant v0.4

- 修改目标：收紧最终 Ozon 标签安全规则，避免中文、斜杠、括号和其他标点进入最终复制清单，并降低报告阅读密度。
- 涉及文件：index.html、css/style.css、js/main.js、docs/manual-test-cases.md、docs/DEVELOPMENT_LOG.md。
- 修改内容：新增 `isOzonFinalTagSafe`，最终标签只允许 `#`、`_`、俄文字母、拉丁字母和数字；最终标签继续使用 `Array.from(tag).length <= 30`；中文词只进入“中文参考 / 需要翻译确认”、候补或明细表，不进入最终清单；移除重复的 `竞品标题（可选）` 输入；标签明细表改为默认折叠；报告左侧保留最终清单、候补和中文参考，右侧放排除原因、评分、竞品/候选分析和数据边界。
- 验收方式：野餐保温包样例中 `保温包`、`大容量` 等中文候选词不进入最终清单；最终标签不含中文、空格、逗号、斜杠、括号或其他标点；`#bag_сумка`、`#женская_сумка`、`#рюкзак`、`#сумка` 等风险标签被排除或标记；浏览器烟测无控制台错误。
- 已知风险：v0.4 仍是本地规则安全过滤，不等于 Ozon 官方标签审核结果；中文到俄语的映射仍需卖家结合平台搜索结果确认。

## 2026-06-09 Refactor / Ozon Topic Tag Strategy Assistant v1.0

- 修改目标：把主题标签助手重构为 `Ozon 主题标签` 策略助手，只处理 Ozon 主题标签，不生成 Wildberries 或 Yandex 关键词逻辑。
- 涉及文件：index.html、css/style.css、js/main.js、docs/manual-test-cases.md、docs/DEVELOPMENT_LOG.md。
- 修改内容：模块标题和边界文案改为 Ozon-only；目标平台固定为 Ozon；报告顶部新增产品类型判断、竞品证据优先级、核心词提取、标签策略建议和最终可复制标签；竞品主题标签优先于系统补充词参与排序；新增汽车内窥镜 / 汽车维修 / 发动机 / 旋转 / 1080P 等工具类规则；产品类型区分工具 / 汽车维修类和普通户外消费品；最终标签继续执行 `#`、`_`、无中文、无不安全标点、`Array.from(tag).length <= 30`；拒绝标签按过泛、错配、中文、禁用/平台词、过长、不安全标点和重复意图分组。
- 验收方式：浏览器测试汽车旋转式内窥镜样例，应判断为工具 / 汽车维修类产品，优先提取 `#эндоскоп` 和竞品核心标签，最终标签不含中文且最长 27 字符；浏览器测试野餐保温包样例，应判断为普通户外消费品，允许场景 / 容量 / 功能扩展，拒绝 `#рюкзак`、`#женская_сумка` 和过泛 `#сумка`；运行 `node --check`、`git diff --check`、前端 Ozon Seller API grep 和占位文案 grep。
- 已知风险：v1.0 仍是本地规则策略助手，不接入 Ozon 官方标签库、搜索量、翻译 API、LLM、爬虫、动态渲染、Worker 或真实凭证；俄语语义、类目边界、型号规格和最终平台审核仍需卖家人工复核。

## 2026-06-12 Refactor / Ozon Topic Tag Candidate Verification Assistant v1.2

- 修改目标：把 `Ozon 主题标签` 从“生成标签方案”进一步调整为“候选标签 + Ozon 搜索验证路径”，避免暗示结果是官方标签。
- 涉及文件：index.html、css/style.css、js/main.js、docs/manual-test-cases.md、docs/DEVELOPMENT_LOG.md。
- 修改内容：移除可见的品牌词 / 禁用词输入，平台词仍由默认内部禁用词过滤；按钮改为更轻的候选标签生成样式；报告新增竞品词拆解、卖家候选词判断、推荐候选标签表、Ozon 搜索验证清单；最终复制区只放高置信候选标签，一行一个；需验证候选标签单独显示并要求先在 Ozon 搜索确认；排除标签继续按原因分组展示。
- 验收方式：汽车内窥镜样例应先分析竞品词，判断为工具 / 汽车维修类，高置信复制区含 `#эндоскоп` 等候选标签，推荐表区分高置信和需验证；野餐保温包样例应判断为普通户外消费品，保留保温包 / 场景 / 容量 / 功能候选，排除 `#рюкзак`、`#женская_сумка`、`#сумка`；两组最终复制标签均为一行一个、无中文、无不安全标点、长度不超过 30。
- 已知风险：v1.2 仍不接入 Ozon 官方标签库、搜索量、翻译 API、LLM、爬虫、动态渲染、Worker 或真实凭证；它只提供候选标签、理由和验证路径，最终标签仍需卖家在 Ozon 搜索结果中确认。

## 2026-06-12 Cleanup / Ozon Topic Tag Assistant v1.2.1

- 修改目标：清理 v1.2 报告密度并收紧高置信候选标签判定，避免把单独泛场景、属性、材质或人群词放进最终复制区。
- 涉及文件：index.html、css/style.css、js/main.js、docs/manual-test-cases.md、docs/DEVELOPMENT_LOG.md。
- 修改内容：竞品标签和卖家候选标签输入改为桌面双列、移动端堆叠；报告顺序调整为产品类型判断、最终复制标签、需验证数量、拒绝数量、推荐候选表、唯一竞品拆解表、Ozon 搜索验证清单、拒绝标签原因；移除独立 `Chinese reference / cannot be used directly as tags` 区；竞品拆解改为 `Tag / Chinese meaning / Type / Source / Status / Action` 表格；高置信逻辑新增强证据和单独弱词排除，`#дом`、`#легкий`、`#хлопок`、`#женский` 不进入最终复制区。
- 验收方式：浏览器测试汽车内窥镜、野餐保温包和包含 `#дом`、`#легкий`、`#хлопок`、`#женский` 的样例；最终复制区只含高置信候选标签，需验证和拒绝标签分区显示；运行 `node --check`、`git diff --check`、前端 Ozon Seller API grep，并确认 Worker 无 diff。
- 已知风险：v1.2.1 仍是本地规则候选标签助手，不接入 Ozon 官方标签库、搜索量、翻译 API、Ozon API、爬虫、动态渲染、Worker 或真实凭证；最终上架前仍需卖家按 Ozon 搜索结果人工确认。

## 2026-06-12 Bugfix / Ozon Topic Tag Assistant v1.2.1 single-status classification

- 修改目标：修复重复来源标签在最终高置信复制区和拒绝明细中同时出现的问题，保证每个 normalized tag 只有一个最终状态。
- 涉及文件：js/main.js、docs/manual-test-cases.md、docs/DEVELOPMENT_LOG.md。
- 修改内容：新增统一 final group key，用同一 normalized key 构建高置信和需验证 accepted set；生成 rejected rows 前过滤已经进入高置信或需验证组的重复来源行，保留 `High confidence > Requires Ozon search verification > Rejected` 的优先级。
- 验收方式：汽车内窥镜样例中 `#эндоскоп` 不再同时出现在高置信和 rejected；野餐保温包样例中 `#термосумка` 不再同时出现在高置信和 rejected；弱词样例中的 `#дом`、`#легкий`、`#хлопок`、`#женский` 仍不进入最终复制区；运行语法检查、`git diff --check`，并确认 Worker 无 diff。
- 已知风险：重复来源信息目前通过最终接受行保留，不再作为 rejected 明细单独展示；真实 Ozon 搜索验证仍需卖家人工执行。
